...
rcbd_mod <- lm(data = rcbd_dfw, 
               yield_kgha ~ nrate_kgha * krate_kgha + rep)
...
```{r rcbd final plot}
ggplot()+
  geom_boxplot(rcbd_dfw,mapping = aes(x=trtname, y=yield_kgha,fill= trtname)) +
  geom_jitter(rcbd_dfw,mapping = aes(x=trtname, y=yield_kgha,color= trtname), alpha = 0.6) +
  geom_label(rcbd_cld_selected,  mapping = aes(x=trtname, y=emmean, label=letter), size = 3) +
